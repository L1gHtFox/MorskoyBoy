...
def shoot_ii(num, matrix, shots, misses):
    ...
    while (row, column) in misses or (row, column) in shots:
        column = random.randint(0, 9)
        row = random.randint(0, 9)
    pos = (row, column)
...
while run:  # основной цикл игры
    ...
    for event in pygame.event.get():
        ...
        if player_one_ready:
            ...
            # двойные свиньи
            if (svins1_pos_num == 4) and (svins2_pos_num < 3):
                angle_svin2 = random.choice([0, 90, 180, 270])
                rotated_svin2 = pygame.transform.rotate(svin2,
            ...
            # тройные свиньи
            if (svins2_pos_num == 3) and (svins3_pos_num < 2):
                angle_svin3 = random.choice([0, 90, 180, 270])
                rotated_svin3 = pygame.transform.rotate(svin3, angle_svin3)
                rotated_msvin3 = pygame.transform.rotate(msvin3, angle_svin3)
                rotated_svin3_bool = True
            ...
            # квадро свиньи
            if (svins3_pos_num == 2) and (svins4_pos_num < 1):
                angle_svin4 = random.choice([0, 90, 180, 270])
        ...
        if player_two_ready and game_end == False:
            if player_one_step:
                ...
                p2_shoot = shoot_ii(2, player_one, player_two_shots, player_two_misses)
                if p2_shoot == 1:
                    print('shots', player_two_shots)
                    print('misses', player_two_misses)
                    # shoot_ii(2, player_one, player_two_shots, player_two_misses)
                elif p2_shoot == 0:
                    strelka = pygame.transform.rotate(strelka, 180)
                    player_two_step = False
                    player_one_step = True
